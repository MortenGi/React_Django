...
class LeaveRoom(APIView):
    def post(self, request, format=None): #actually POST just used when creating something new
...
class UpdateRoom(APIView):
    ...
    def patch(self, request, formate=None): #patch used to update something
        if not self.request.session.exists(self.request.session.session_key):
            self.request.session.create()
